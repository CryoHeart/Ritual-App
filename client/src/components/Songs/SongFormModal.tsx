...
import type { Song, UpdateSongRequest } from '../../types/song';
...
interface SongFormModalProps {
  ...
  onClose: () => void;
  onSubmit: (payload: UpdateSongRequest) => Promise<void>;
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/80 px-4 backdrop-blur-sm">
      <RitualCard className="w-full max-w-2xl border-zinc-700/90">
        ...
        <h3 className="mt-2 text-2xl font-semibold text-zinc-100">Edit Song</h3>
        ...
        <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            ...
            <label className="block">
              ...
              >
                ...
                {selectableAlbums.map(album => (
                  <option key={album.albumId ?? 'album'} value={album.albumId ?? ''}>
                    {album.title}
                  </option>
                ))}
              </select>
          ...
          {(formError || submitError) && <p className="text-sm text-red-300">{formError ?? submitError}</p>}

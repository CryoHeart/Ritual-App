...
export function CeremonyModePage({ setlist, bandName, onEnd }: CeremonyModePageProps) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_15%_-10%,rgba(127,29,29,0.32),transparent_42%),radial-gradient(circle_at_90%_0%,rgba(39,39,42,0.4),transparent_40%),#050506] text-zinc-100">
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-8 lg:px-10">
        <header className="mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-zinc-800/80 pb-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.38em] text-zinc-300">Ritual</p>
            <h1 className="mt-2 text-4xl font-semibold tracking-tight text-white lg:text-5xl">Ceremony Mode</h1>
            <p className="mt-2 text-sm text-zinc-400">Live stage control and song progression.</p>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {bandName ? <RitualBadge tone="accent">Band: {bandName}</RitualBadge> : null}
            <RitualBadge>{setlist.name}</RitualBadge>
            <RitualBadge>{totalSongs} songs</RitualBadge>
          </div>
        </header>
        ...
        {isEmpty ? (
          <RitualCard className="flex flex-1 flex-col items-center justify-center gap-6 rounded-2xl px-8 py-16 text-center">
            ...
            <p className="max-w-xl text-zinc-400">Add songs to the setlist in Dashboard to begin Ceremony Mode.</p>
